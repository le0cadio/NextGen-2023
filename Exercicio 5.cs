...
{
    public static int[] CalculaPercentualDeSorteio(bool[] assinante, int[] minutosAssistidos)
    {
        int[] horasAssistidas = minutosAssistidos.Select(x => x / 60).ToArray();
        int totalHoras = horasAssistidas.Sum();
        int[] horasAssistidasPremium = horasAssistidas
            .Select((x, i) => assinante[i] ? x * 2 : x)
            .ToArray();
        int totalHorasPremium = horasAssistidasPremium.Sum();
        return horasAssistidasPremium.Select(x => x * 100 / totalHorasPremium).ToArray();
    }
}
