...
public class Challenge
{
    ...
    {
        var musicas = musicasTocadas
            .Select((x, i) => (x, i))
            .OrderByDescending(x => x.x)
            .Select(x => x.i)
            .ToArray();
        var resultado = new int[musicas.Length];
        var index = 0;
        for (int i = 0; i < musicas.Length; i++)
        {
            if (i % 2 == 0)
            {
                resultado[i] = musicasTocadas[musicas[index]];
                index++;
            }
            else
            {
                resultado[i] = musicasTocadas[musicas[musicas.Length - index]];
            }
        }
        return resultado;
    }
